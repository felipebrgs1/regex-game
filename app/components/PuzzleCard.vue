...
const matchResults = computed((): WordResult[] => {
  if (!compiledRegex.value) {
    return [
      ...props.puzzle.shouldMatch.map((w) => ({
        word: w,
        expectedMatch: true,
        actualMatch: false,
        isCorrect: false,
      })),
      ...props.puzzle.shouldNotMatch.map((w) => ({
        word: w,
        expectedMatch: false,
        actualMatch: false,
        isCorrect: true,
  ...
  >
    ...
    <div class="mb-8">
      ...
      <div class="relative group/input">
        <div
          class="absolute left-4 top-1/2 -translate-y-1/2 text-rosewater font-black text-lg select-none"
        >
          /
        </div>
        <input
      ...
      <div
        v-if="regexError"
        class="mt-2.5 flex items-center gap-2 p-3 rounded-lg bg-red/10 border border-red/20"
      >
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-wrap gap-2.5">
          ...
          >
            {{ result.word }}
            <span
              class="text-[10px] flex items-center justify-center w-4 h-4 rounded-full bg-white/50 border border-current/20"
              >{{ wordIcon(result) }}</span
            >
          </span>
        </div>
      </div>
